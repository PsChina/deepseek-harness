...
export class AgentPresets extends TypertRemoteService {
  ...
  get defaultId(): string {
    ...
    return canonicalPresetId(this.selectionPolicy().defaultId)
  }
  ...
  async resolve(id?: string): Promise<AgentPreset> {
    ...
    if (found === undefined) {
      ...
      throw new RemoteError(
        ...
        `agent-presets: preset "${requested}" not found (available: ${available.join(', ') || 'none'})`,
        { agentPreset: requested, available },
      )
    }
    return found
  }
