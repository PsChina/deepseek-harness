...
`guard/` 组监视 agent loop（智能体循环）中的三种失败模式。`turn-continuation` 把因输出 token 上限被截断的轮次当中断而非终点，在下一次空闲点续行该工作，让被截断的回答无需催促即可完成。`repeat-tool-reminder` 在模型重复相同工具调用时提醒它改变方法或结束任务，让卡住的循环不再浪费 token。`timeout-policy` 为声明了限时的工具调用设置超时，返回超时错误而不是拖住会话。三者都随 `dsh` base 组合默认启用；组合可以调优或移除它们。
