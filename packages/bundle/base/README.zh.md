...
Qwen 服务器为 Q3 提供 `92,000` token、为 Q2 提供 `192,000` token 的上下文容量，而 profile 在 Harness 中分别按 `82,000` 和 `170,000` token 预算运行。Q3 的请求和摘要输出上限为 `9,216` token，压缩余量为 `7,184` token，因此压力触发线为 `65,600` token、保留历史区为 `11,645` token。Q2 的请求和摘要输出上限为 `16,384` token，压缩余量为 `17,616` token，因此压力触发线为 `136,000` token、保留历史区为 `24,578` token。其他路由仍使用共享的 `65,536` token 默认值，除非被覆盖。
