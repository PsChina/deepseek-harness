...
Qwen 服务器为 Q3 提供 `92,000` token 上下文容量；Q2 则配置为两个并发槽共用 `190,000` token 池。profile 在 Harness 中为 Q3 预算 `82,000` token，为每个 Q2 请求预算 `100,000` token，与 Q2 服务端的单请求限制一致。Harness 的思考档位菜单是通用的，但这两个 Qwen 模型条目只开放 `off`、`low`、`medium`、`xhigh`：三个原生 Qwen 思考档位原样发送，`off` 则是 Harness 增加的关闭思考开关，映射为 `none`。两条路由都默认 `xhigh`。Qwen 部署栈的启动器保留官方思考模式采样参数作为服务端默认值，Harness 也会在每个请求中发送对应的官方预设：`temp=1.0`、`top_p=0.95`、`top_k=20`、`min_p=0`、`presence_penalty=0`、`repeat_penalty=1`。选择 `off` 时则发送 Qwen 独立的非思考预设（`temp=0.7`、`top_p=0.80`、`top_k=20`、`min_p=0`、`presence_penalty=1.5`、`repeat_penalty=1`）。Q3 普通请求的 `9,216` token 上限（约 9K）是每次生成请求的输出上限，不是上下文或思考档位设置，也不是整个回合的总上限：自动续写可以再次发起请求，并继续受同一上限约束。Q3 另有独立的 `55,000` token 摘要上限及 `7,184` token 压缩余量，因此压力触发线为 `65,600` token、保留历史区为 `11,645` token。Q2 的普通请求上限为 `16,384` token，摘要有独立的 `100,000` token 上限，压缩余量为 `3,616` token，因此压力触发线为 `80,000` token、保留历史区为 `13,378` token。在 Qwen 部署栈中，摘要上限只是最大值；代理上下文保护器会按渲染后提示词的 token 数及剩余生成容量进一步降低摘要请求的输出上限。其他路由仍使用共享的 `65,536` token 默认值，除非被覆盖。上游设置见 [Qwen3.8-27B 模型卡](https://huggingface.co/Qwen/Qwen3.8-27B)。
