...
Qwen 服务器为 Q3 提供 `92,000` token 上下文容量；Q2 则配置为两个并发槽共用 `190,000` token 池。profile 在 Harness 中为 Q3 预算 `82,000` token，为每个 Q2 请求预算 `100,000` token，与 Q2 服务端的单请求限制一致。Q3 的普通请求上限为 `9,216` token，摘要有独立的 `55,000` token 上限，压缩余量为 `7,184` token，因此压力触发线为 `65,600` token、保留历史区为 `11,645` token。Q2 的普通请求上限为 `16,384` token，摘要有独立的 `100,000` token 上限，压缩余量为 `3,616` token，因此压力触发线为 `80,000` token、保留历史区为 `13,378` token。在 Qwen 部署栈中，摘要上限只是最大值；代理上下文保护器会按渲染后提示词的 token 数及剩余生成容量进一步降低摘要请求的输出上限。其他路由仍使用共享的 `65,536` token 默认值，除非被覆盖。
