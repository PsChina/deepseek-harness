...
The Qwen servers support `92,000` context tokens for Q3 and a configured `190,000`-token shared pool for two Q2 slots. The profile budgets `82,000` for Q3 and `100,000` per Q2 request in Harness, matching the Q2 server-side request limit. The Harness effort picker is generic, but these Qwen model entries expose only `off`, `low`, `medium`, and `xhigh`; the three native Qwen thinking efforts pass through unchanged, while `off` is a Harness switch sent as `none` to disable thinking. Both routes default to `xhigh`. The Qwen stack launchers keep the official thinking-mode sampler as their server default, and Harness sends the matching official preset on each request: `temp=1.0`, `top_p=0.95`, `top_k=20`, `min_p=0`, `presence_penalty=0`, `repeat_penalty=1`. Selecting `off` sends Qwen's separate non-thinking preset instead (`temp=0.7`, `top_p=0.80`, `top_k=20`, `min_p=0`, `presence_penalty=1.5`, `repeat_penalty=1`). Q3's `9,216`-token ordinary-request cap (about 9K) is a per-generation-request output ceiling, not a context or reasoning-effort setting and not a whole-turn ceiling: automatic continuation can issue another request under the same cap. Q3 also uses a separate `55,000`-token summary cap and `7,184` tokens of compaction headroom, giving it a `65,600`-token pressure threshold and an `11,645`-token retained tail. Q2 uses a `16,384`-token ordinary-request cap, a separate `100,000`-token summary cap, and `3,616` tokens of headroom, giving it an `80,000`-token pressure threshold and a `13,378`-token retained tail. Summary caps are upper bounds; in the Qwen stack, the proxy context governor may lower a summary request's output cap to fit the generation capacity remaining after tokenizing its rendered prompt. Other routes use the shared `65,536`-token default unless overridden. See the [Qwen3.8-27B model card](https://huggingface.co/Qwen/Qwen3.8-27B) for the upstream settings.
