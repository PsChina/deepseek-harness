...
describe('TypeScript SDK snapshots over the jsonrpc runtime', () => {
  it('restores the retained V3 max-tokens recording after an interrupted next-turn restart', async () => {
    ...
    // Removing or replacing the turn/end reflows every later row seq, so the
    // recorded sourceEventSeqs no longer point at the rows they described. The
    // assertion targets the interrupted-turn recovery, not source references.
    const serialize = (values: JsonObject[]) => values
      .map(row => { const copy: JsonObject = { ...row }; delete copy['sourceEventSeqs']; return JSON.stringify(copy) })
      .join('\n') + '\n'
